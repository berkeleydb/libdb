...
#ifndef lint
static const char sccsid[] = "@(#)os_map.c	11.5 (Sleepycat) 10/31/99";
#endif /* not lint */
...
static int __os_map
    __P((char *, REGINFO *, DB_FH *, size_t, int, int, int, void **));
	...
	REGION *rp;
{
	...
	/* Map the file in. */
	ret = __os_map(infop->name, infop, &fh, rp->size,
	...
	int destroy;
{
	...
	if (F_ISSET(dbenv, DB_ENV_SYSTEM_MEM) && destroy &&
	    (t_ret = __os_unlink(infop->name)) != 0 && ret == 0)
		ret = t_ret;
	...
	void **addr;
{
	...
	return (__os_map(path, NULL, fhp, len, 0, 0, is_rdonly, addr));
}
 ...
 */
static int
__os_map(path, infop, fhp, len, is_region, is_system, is_rdonly, addr)
	REGINFO *infop;
	char *path;
	DB_FH *fhp;
	...
	void **addr;
{
	...
	/* Sharing via the paging file does not work with NT. */
	use_pagefile = is_region && is_system && !__os_is_winnt();
