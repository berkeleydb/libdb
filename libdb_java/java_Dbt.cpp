...
#ifndef lint
static const char sccsid[] = "@(#)java_Dbt.cpp	11.2 (Sleepycat) 8/26/99";
#endif /* not lint */
...
{
	DBT_javainfo *dbt = NEW(DBT_javainfo);
...
{
	DBT_javainfo *db_this = get_DBT(jnienv, jthis);
...
JNIEXPORT void JNICALL Java_com_sleepycat_db_Dbt_finalize
  (JNIEnv *jnienv, jobject jthis)
{
	DBT_javainfo *dbt = get_DBT(jnienv, jthis);
	if (dbt) {
