...
typedef struct __db_compact {
	...
	%typemap(csvarout) db_timeout_t    compact_timeout %{%}
	db_timeout_t	compact_timeout;	/* Lock timeout. */
	%typemap(csvarout) u_int32_t compact_pages %{%}
	u_int32_t	compact_pages;		/* Max pages to process. */
	/* Output Stats. */
	%typemap(csvarin) u_int32_t compact_empty_buckets %{%}
	u_int32_t	compact_empty_buckets;	/* Empty hash buckets found. */
	%typemap(csvarin) u_int32_t compact_pages_free %{%}
	u_int32_t	compact_pages_free;	/* Number of pages freed. */
	%typemap(csvarin) u_int32_t compact_pages_examine %{%}
	u_int32_t	compact_pages_examine;	/* Number of pages examine. */
	%typemap(csvarin) u_int32_t compact_levels %{%}
	u_int32_t	compact_levels;		/* Number of levels removed. */
	%typemap(csvarin) u_int32_t compact_deadlock %{%}
	u_int32_t	compact_deadlock;	/* Number of deadlocks. */
	%typemap(csvarin) u_int32_t compact_pages_truncated %{%}
...
%extend {
	...
	%typemap(cstype) u_int32_t (*)(DB*, const DBT*, const DBT*) "BDB_PrefixCompareDelegate"
	%typemap(imtype) u_int32_t (*)(DB*, const DBT*, const DBT*) "BDB_PrefixCompareDelegate"
	%typemap(csin) u_int32_t (*callback)(DB *dbp, const DBT *dbt1, const DBT *dbt2) "callback"
	int set_bt_prefix(size_t (*callback)(DB *dbp, const DBT *dbt1, const DBT *dbt2)) {
...
typedef struct __db_repmgrsite
{
	...
	int eid;
	%typemap(csvarin) char host[] %{%}
	char host[];
	%typemap(csvarin) u_int port %{%}
	u_int port;
	%typemap(csvarin) u_int32_t status %{%}
	u_int32_t status;
...
typedef struct __key_range {
	...
	double equal;
	%typemap(csvarin) double greater %{%}
	double greater;
} DB_KEY_RANGE;
