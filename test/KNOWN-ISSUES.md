...
| id | issue | status |
|----|-------|--------|
| **P9** | **A second scaling defect exists that is not the log.** With the log removed from the append path entirely (`DB_TXN_NOT_DURABLE`), throughput *still* falls from 235k at t=8 to 131k at t=96. So P5 was never the whole story. The most promising lead, from RFC 0008's measurements, is **mutex acquisition cost itself**: mean hold time inflates 21ns → ~300ns (14×) with thread count for *constant work*, the default spin count is `ncpu × MUTEX_SPINS_PER_PROCESSOR(50)` = **4,800 TAS attempts on one cacheline** per acquisition (`mutex.h:30`), and every successful acquire writes `mutexp->pid`/`tid` to that same line (`mut_tas.c:204-205`). An early probe at `tas_spins=1` was *up* at t=4 (221k vs ~170k) and t=16 (113k vs ~90k) — pure configuration, no code change, no format break. This would benefit **every** subsystem, not just the log, which likely makes it higher-value than any log-specific work. | open |
