	...
	void *info;
{
	...
	PAGE *hdrstart;
	int32_t inttmp;
	u_int32_t hdrsize, i, op, uinttmp;
	...
	for (sp = spec; sp->type != LOGREC_Done; sp++) {
		...
		case LOGREC_OP:
			...
			(void)printf(sp->fmt, OP_MODE_GET(op));
			(void)printf(" ptype: %s\n",
		...
		case LOGREC_DBT:
			...
			case LOGREC_DATA:
				if (downrev ? LOG_SWAPPED(env) :
					...
					__db_recordswap(op, hdrsize, hdrstart,
					    (has_data && uinttmp != 0) ?
					    bp : NULL, 1);
				(void)printf("\t%s: ", hdrname);
				for (i = 0; i < hdrsize; i++) {
					ch = ((u_int8_t *)hdrstart)[i];
					printf(isprint(ch) ||
					ch == 0x0a ?
					    "%c" : "%#x ", ch);
				}
			...
			case LOGREC_PGDDBT:
				...
				if (downrev)
					goto pr_data;
				if ((ret = __db_prpage_int(env, dbp, "\t",
				     hdrstart, uinttmp, has_data == 0 ?
				     NULL : bp, DB_PR_PAGE)) != 0)
					return (ret);
				has_data = 0;
				break;
			case LOGREC_PGLIST:
				dbt.data = bp;
				dbt.size = uinttmp;
				__db_pglist_print(&dbt);
				break;
			case LOGREC_LOCKS:
				dbt.data = bp;
				dbt.size = uinttmp;
				__lock_list_print(env, &dbt);
				break;
			}
			bp += uinttmp;
			break;
		...
		case LOGREC_POINTER:
			LOGCOPY_TOLSN(env, &prev_lsn, bp);
			(void)printf("\t%s: [%lu][%lu]\n", sp->name,
			    (u_long)prev_lsn.file,
			    (u_long)prev_lsn.offset);
			bp += sizeof(DB_LSN);
			break;
		case LOGREC_Done:
