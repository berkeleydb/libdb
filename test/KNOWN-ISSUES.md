...
| id | issue | status |
|----|-------|--------|
| **P4** | **`__db_walk_cursors` takes the environment-wide `env->mtx_dblist` on every B-tree insert.** With P1 fixed, this is now the dominant serialization point on the write path. `__bam_iitem` → `__bam_ca_di` → `__db_walk_cursors` (`src/db/db.c:1381`) acquires **one global mutex** and then walks the `active_queue` of **every open cursor on every handle in the environment** to adjust indices after an insert. Profiled at t=32 on a production (non-DIAGNOSTIC) build: `__bam_ca_di` is **25.58%** of all time, the single largest term, inside a total of 62.32% `__db_tas_mutex_lock_int` + 10.27% `__db_tas_mutex_unlock`. Measured shape on 96 vCPU after P1: peak **203,653 ops/s at t=2**, then monotonic decline to **103,669 at t=96** — a **49% loss**, and the reason throughput never climbs. This is *not* the "cliff after 8 threads" it was described as; it is a ceiling from t=4 onward with no knee, which is the signature of a single global lock rather than a partitioned one. Note the cost is paid even when no other cursor could possibly be affected. Candidate directions: skip the walk when the handle has no other open cursors; key the walk off the affected page rather than scanning all cursors; or move cursor adjustment off `mtx_dblist` onto a per-handle lock. | open, root-caused |
