...
#
#	@(#)test.tcl	10.19 (Sleepycat) 1/17/98
...
global recdtests
	exec $RM -rf ALL.OUT
	foreach i "DB_BTREE DB_HASH DB_RECNO DB_RRECNO" {
	...
	foreach i "archive byte lock log mpool mutex txn" {
		puts "Running $i tests"
...
# If recno-with-renumbering is specified, then fix the arguments to specify
# the DB_RENUMBER option for the -flags argument.
...
source ./include.tcl
	if { [string compare $method "DB_BTREE"] == 0 } {
		...
		if { $ndx >= 0 } {
			...
			incr ndx
			set f [lindex $largs $ndx]
			set f [expr $f | $DB_RECNUM]
			set largs [lreplace $largs $ndx $ndx $f]
		} else {
			# There is no flags argument
			lappend largs -flags $DB_RECNUM
		}
	}
	return $largs
}
