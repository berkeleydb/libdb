...
#ifndef lint
static const char sccsid[] = "@(#)java_util.cpp	10.2 (Sleepycat) 1/18/98";
#endif /* not lint */
...
{
    if (obj == NULL) {
        report_exception(jnienv, "null object");
        return 0;
    }
    return 1;
}
...
int verify_return(JNIEnv *jnienv, int err)
{
    ...
    if (err != 0) {
        if (err > 0 && (errstring = strerror(err)) != NULL) {
            report_exception(jnienv, errstring);
        }
        else {
            char buffer[64];
            sprintf(buffer, "error %d", err);
            report_exception(jnienv, buffer);
        }
        return 0;
    }
    return 1;
...
{
    return (DBC *)get_private_info(jnienv, name_DB, obj);
}
...
{
    return (DB_MPOOLFILE *)get_private_info(jnienv, name_DB_MPOOLFILE, obj);
}
    ...
    /* dbt initialized below */
{
    ...
    if ((dbt->flags & DB_DBT_USERMEM) || !is_retrieve_op_) {
        ...
        //
        if (!dbt->array_) {
            report_exception(jnienv, "Dbt.data is null");
            has_error_ = 1;
            return;
        }
        ...
        if (dbt->offset_ < 0 ) {
            report_exception(jnienv, "Dbt.offset illegal");
            has_error_ = 1;
            return;
        }
        if (dbt->ulen + dbt->offset_ > java_array_len_) {
            report_exception(jnienv,
                           "Dbt.ulen + Dbt.offset greater than array length");
            has_error_ = 1;
            return;
        }

        jbyte *data = jnienv->GetByteArrayElements(dbt->array_, NULL);
        dbt->data = data + dbt->offset_;
    }
    else {
...
LockedDBT::~LockedDBT()
{
    ...
    if ((dbt->flags & DB_DBT_USERMEM) || !is_retrieve_op_) {
        ...
        // If writing with DB_DBT_USERMEM or reading, then the data
        // is already in the java array, we just need to release it.
        //
        jbyte *data = (jbyte *)dbt->data;
        data -= dbt->offset_;
        env_->ReleaseByteArrayElements(dbt->array_, data, 0);
        dbt->data = 0;
    }
    else {
