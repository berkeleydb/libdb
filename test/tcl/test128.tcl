...
proc test128 {method { nentries 10000 } args } {
	source ./include.tcl
	...
	eval {test126 $method $nentries "128" -subdb} $args
	eval {verify_dir $testdir "" 1 0 $nodump}
	eval {salvage_dir $testdir "" 1}

	# Test using secondary database
	eval {test126 $method $nentries "128" -secondary} $args
	eval {verify_dir $testdir "" 1 0 $nodump}
	eval {salvage_dir $testdir "" 1}

	# Test using both sub database and secondary database
	eval {test126 $method $nentries "128" -secondary -subdb} $args
