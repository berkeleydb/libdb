...
| id | issue | status |
|----|-------|--------|
| **P1** | **Root cause found 2026-09-19: `LOCK_LOCKERS` acquires all 64 locker stripes on every transaction begin AND end, defeating the striping it was given.** Profiled at t=96 on 96 vCPU: 43.65% `__db_tas_mutex_lock_int` + 29.92% kernel spinlock slowpath, of which `__lock_getlocker` (txn_begin) and `__lock_freelocker` (txn_end) are **~30% of all time**, blocking in `__db_pthread_mutex_condwait` — a *sleeping* mutex. `lock.h:400` defines `LOCK_LOCKERS` as a serial loop over all `LOCK_LOCKER_STRIPES` (64) `mtx_locker_stripe[]` entries, while a per-id `RDLOCK_LOCKER(env, region, id)` using `LOCK_LOCKER_STRIPE(id)` already exists at `lock.h:396`. In `__lock_id` (`lock_id.c:72-104`) the common path under that global lock is only `id = ++region->lock_id` plus an insert into ONE stripe; the all-stripes hold is needed solely for the rare id-wraparound scan (`lock_id.c:83-97`). **The earlier `PGNO_BASE_MD` attribution was wrong — `__db_new` does not appear in the profile at all.** Partitioning knobs do not help (`set_mp_mtxcount=4096` and `set_lk_partitions=960` are both within noise), confirming this is a code defect, not a defaults problem. Reproduced: 46,054 ops/s @t=1, peak 53,921 @t=8, 46,196 @t=96. Fix: take one stripe on the common path, all stripes only on wraparound. | open, root-caused |
