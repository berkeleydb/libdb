...
#ifndef lint
static const char revid[] = "$Id: bt_cursor.c,v 11.114 2001/07/03 16:52:55 sue Exp $";
#endif /* not lint */
	...
	case DB_GET_BOTH:
		...
		if (F_ISSET(dbc, DBC_OPD)) {
			if ((ret = __bam_c_search(
			    dbc, PGNO_INVALID, data, DB_GET_BOTH, &exact)) != 0)
				goto err;
			if (!exact) {
				ret = DB_NOTFOUND;
				goto err;
			...
			if ((ret = __bam_c_search(
			    dbc, PGNO_INVALID, key, DB_GET_BOTH, &exact)) != 0)
				return (ret);
			if (!exact) {
				ret = DB_NOTFOUND;
				goto err;
