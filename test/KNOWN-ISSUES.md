...
| id | issue | status |
|----|-------|--------|
| **P9** | **RETRACTED — `tas_spins=1` does not hang, and the mechanism previously recorded here was false.** The earlier entry claimed 3-of-4 runs hang at t=32 on 96 vCPU, caused by `__db_hybrid_mutex_suspend` asserting `DB_MUTEX_SELF_BLOCK`, "a flag never set at allocation". **Both halves are wrong.** (1) *Not reproducible:* on a dedicated 96 vCPU `c6i.24xlarge` (DIAGNOSTIC on, `HAVE_MUTEX_HYBRID` on), `tas_spins=1` completed **12/12** runs — 6 reps of `p5_log_bench` at t=32 (88-93k ops/s, no outliers) plus 3 reps each at t=32 and t=96 of a purpose-built txn driver — and a further 20+ runs on 32 vCPU across t={8,16,32,64,96} and all three durability modes. Zero hangs, zero assertions. (2) *Mechanism disproven by reading the code:* hybrid allocation **does** set the flag — `mut_tas.c:60-61` calls `__db_pthread_mutex_init(env, mutex, flags | DB_MUTEX_SELF_BLOCK)`, and `mut_pthread.c:164` does `F_SET(mutexp, DB_MUTEX_SELF_BLOCK)`. The assertion at `mut_pthread.c:561` therefore holds by construction. The original hang was real (rc=124 was observed) but was **never isolated to this knob**: it was measured with an ad-hoc patch on a shared box, and `scale_bench` has no spins knob, so the run that produced it is not reconstructible from committed code. Recorded here rather than deleted because the retraction is the finding: *a hang observed once, attributed by reading code instead of testing the attribution, became a documented mechanism that does not exist.* Any future spin-reduction work should proceed — this is not a blocker. | **retracted, not a defect** |
...
| id | issue | status |
|----|-------|--------|
| **P6** | **FIXED.** `DB_BACKUP_NO_LOGS` was accepted and silently ignored: it appeared exactly twice in the tree (its `#define` and `db_backup.c`'s accepted-flag mask) and was read nowhere, so a backup taken with the flag copied all 62 log files. Fixed by guarding the `backup_read_log_dir` call. `copy_min` stays 0 when logs are skipped, and the `DB_BACKUP_UPDATE` consistency test is `remove_max < copy_min`, so the two flags do not interact badly. Measured after: 2 db files and **0** log files, against a control backup with 62. | **fixed** |
| **P7** | **FIXED.** `DB_INORDER` + `DB_CONSUME` across a deleted record hung forever at 100% CPU with no concurrency. Root cause: the local `first` advances past records confirmed gone, but the persistent `meta->first_recno` only advances when a consume *succeeds*, and the `is_first` guard demanded equality with the persistent value alone -- so `retry` re-entered `DB_FIRST`, which reset `cp->recno` to the hole just skipped (traced oscillation 10,11,10,11,...). Fix accepts either notion of the head; when nothing was skipped the two are equal and the test is unchanged. Regression `test/db/run_qam_inorder_consume.sh` judges on a VERDICT line rather than rc (the defect is a hang), with teeth proven both ways on both the `timeout` and poll-fallback paths. PR #205. | **fixed** |
| **P8** | **FIXED.** `DB_NOFLUSH` made an environment unusable (SIGBUS on shared, `DB_PAGE_NOTFOUND` on private). `LAST_PANIC_CHECK_BEFORE_IO` tested the **public** `DB_ENV_NOFLUSH` and expanded to an unconditional `return (0)` inside `__os_physwrite`'s write loop and both arms of `__os_io`, so every read and write reported success and moved no data. Present **verbatim since Oracle 5.1.29**. The clause is load-bearing for panic teardown (`env_open.c` sets `NOPANIC|NOFLUSH`, and `NOPANIC` makes `PANIC_ISSET` false, so `PANIC_CHECK` alone would not stop the I/O), so it could not simply be deleted: it now tests a new internal-only `DB_ENV_NOIO` (`0x00400000`, a process-local `dbenv->flags` bit — no region or ABI impact). `DB_ENV_NOFLUSH` keeps its one real meaning, "do not flush the cache on close", which was already implemented at `env_open.c:844`. | **fixed** |
