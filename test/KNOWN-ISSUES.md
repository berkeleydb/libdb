...
| id | issue | status |
|----|-------|--------|
| **P1** | **The `PGNO_BASE_MD` attribution is WRONG — corrected 2026-09-19 by direct profiling.** `__db_new` does hold the metadata page to commit (`__TLPUT` is a no-op for a write lock in a txn), and that remains true — but it is **not** where the time goes. A dwarf-call-graph profile of a pure insert workload at t=96 puts **43.65% in `__db_tas_mutex_lock_int` and 29.92% in the kernel's spinlock slowpath**, attributed as: `__memp_fget` **10.91%** + `__memp_fput` **9.90%** (buffer-pool hash mutexes), `__lock_freelocker` **6.11%** + `__lock_getlocker` **5.88%** (locker-table mutex), `__bam_get_root` 4.72%. **`__db_new` does not appear in the profile at all** (0 frames above 0.1%). The earlier WAIT-census claim ("400 of 400 waits on page 0") measured *lock-manager* waits, which are real but brief: `st_lock_wait` rises 0 → 14,986 from t=1 → t=96 while `db_stat -Cl` sampling finds **zero standing WAIT records**, so those waits are not the stall. Reproduced negative scaling: **46,054 ops/s @t=1, peak 53,921 @t=8, 53,250 @t=32, 46,196 @t=96**. So the real target is **mutex contention in mpool and the locker table**, not metadata-page lock scope — and the two prior attempts failed partly because they were aimed at the wrong subsystem. | open, retargeted |
