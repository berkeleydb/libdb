...
source: docs/gsg_db_rep/CXX/elections.html
---
## Holding Elections
...
Manually selecting a master is an activity that should be performed infrequently, if ever. You turn elections off by using the `DbEnv::rep_set_config()` and `DbEnv::repmgr_start()` methods.
