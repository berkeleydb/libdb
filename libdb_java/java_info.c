				     ...
				     DB_LSN *lsn, db_recops recops)
{
	...
	DB_ASSERT(dbenv != NULL);
	dbinfo = (DB_ENV_JAVAINFO *)dbenv->cj_internal;
	return dbjie_call_tx_recover(dbinfo, dbenv, dbinfo->jenvref, dbt,
				     lsn, recops);
}
		...
		int is_dbopen)
{
	...
	if ((*jnienv)->GetJavaVM(jnienv, &dbjie->javavm) != 0) {
		__os_free(NULL, dbjie, sizeof(DB_ENV_JAVAINFO));
...
{
	if (dbjie->recovery_init != NULL) {
		DELETE_GLOBAL_REF(jnienv, dbjie->recovery_init);
		dbjie->recovery_init = NULL;
	}
	if (dbjie->feedback != NULL) {
		DELETE_GLOBAL_REF(jnienv, dbjie->feedback);
		dbjie->feedback = NULL;
	}
	if (dbjie->tx_recover != NULL) {
		DELETE_GLOBAL_REF(jnienv, dbjie->tx_recover);
		dbjie->tx_recover = NULL;
	}
	...
	if (dbjie->conflict != NULL) {
		__os_free(NULL, dbjie->conflict, dbjie->conflict_size);
		dbjie->conflict = NULL;
		dbjie->conflict_size = 0;
	}
	if (dbjie->errpfx != NULL) {
		__os_freestr(NULL, dbjie->errpfx);
		dbjie->errpfx = NULL;
	}
}
...
void dbjie_destroy(DB_ENV_JAVAINFO *dbjie, JNIEnv *jnienv)
{
	...
	/* Extra paranoia */
	memset(dbjie, 0, sizeof(DB_ENV_JAVAINFO));
	(void)__os_free(NULL, dbjie, sizeof(DB_ENV_JAVAINFO));
}
...
dbjie_get_jnienv(DB_ENV_JAVAINFO *dbjie)
{
	 ...
	 */
	if ((*dbjie->javavm)->AttachCurrentThread(dbjie->javavm, &attachret, 0) != 0)
		return (0);
...
void
dbjie_set_conflict(DB_ENV_JAVAINFO *dbjie, unsigned char *newarr, size_t size)
{
	if (dbjie->conflict != NULL)
		(void)__os_free(NULL, dbjie->conflict, dbjie->conflict_size);
				    ...
				    jobject jrecovery_init)
{
	...
	dbjie->recovery_init = NEW_GLOBAL_REF(jnienv, jrecovery_init);
}
			     ...
			     jobject jenv)
{
	...
	return (*jnienv)->CallIntMethod(jnienv, dbjie->recovery_init,
					id, jenv);
}
				 ...
				 DB_ENV *dbenv, jobject jtx_recover)
{
	...
	dbjie->tx_recover = NEW_GLOBAL_REF(jnienv, jtx_recover);
}
			  ...
			  DBT *dbt, DB_LSN *lsn, int recops)
{
	...
	return (*jnienv)->CallIntMethod(jnienv, dbjie->tx_recover, id, jenv,
					jdbt, jlsn, recops);
}
...
dbji_destroy(DB_JAVAINFO *dbji, JNIEnv *jnienv)
{
	...
	__os_free(NULL, dbji, sizeof(DB_JAVAINFO));
}
...
JNIEnv *dbji_get_jnienv(DB_JAVAINFO *dbji)
{
	 ...
	 * some Java activity.  I think therefore I am (a thread).
	 */
	if ((*dbji->javavm)->AttachCurrentThread(dbji->javavm, &attachret, 0) != 0)
		return (0);
				  ...
				  DBT *dbt, jint recno)
{
	 ...
	 */
	if ((err = locked_dbt_get(&lresult, jnienv, jresult, inOp)) != 0)
		return (err);

	memcpy(dbt, &lresult.javainfo->dbt, sizeof(DBT));
	if ((err = __os_umalloc(db->dbenv, dbt->size, &bytearray)) != 0)
		goto out;
			       ...
			       jobject jcallback, int flags)
{
	...
	if (dbji->assoc_method_id == NULL) {
		assoc_class = get_class(jnienv, name_DbSecondaryKeyCreate);
	...
	if (jcallback == NULL)
		err = db->associate(db, second, NULL, flags);
	else
		err = db->associate(db, second, Db_assoc_callback, flags);
			   ...
			   const DBT *key, const DBT *value, DBT *result)
{
	...
	if (sz > 0) {
		bytearray = (unsigned char *)lresult.javainfo->dbt.data;
		 ...
		 */
		if ((bytearray < (unsigned char*)key->data ||
		     bytearray + sz > (unsigned char*)key->data + key->size) &&
		    (bytearray < (unsigned char*)value->data ||
		     bytearray + sz > (unsigned char*)value->data + value->size)) {

			result->flags |= DB_DBT_APPMALLOC;
			if ((err = __os_umalloc(db->dbenv, sz, &bytearray)) != 0)
				goto out;
