...
available in Berkeley DB version 2. In general it is safe to assume
...
function B<db_appinit>. Its purpose is to initialise a number of
...
Add extra dubugging information to the messages sent to B<-ErrFile>.
...
The equivalent of B<db_open> with type B<DB_HASH>. Two forms of
...
invert the fruit/colour hash

=head2 Sorting Duplicate Keys

=head1 BerkeleyDB::Btree

The equivalent of B<db_open> with type B<DB_BTREE>. Two forms of
constructor are supported:
...
Apart from the standard Methods, B<BerkeleyDB::Btree> supports one
extra method:
...
=head2 A Btree Example

=head2 Changing the sort order

This script shows how to override the default sorting algorithm that
BerkeleyDB::Btree uses. Instead of using the normal lexical ordering,
a case insensitive compare function will be used.
...
The equivalent of B<db_open> with type B<DB_RECNO>. Two forms of
constructor are supported:
...
The equivalent of B<db_open> with type B<DB_UNKNOWN>. The constructor
...
The new key/value pair will be stored 
...
See the MLDNM documentation for information on how to use the module
...
This section answers some of the questions that have arisen
...
Copyright (c) 1997-1999 Paul Marquess. All rights reserved. This program
