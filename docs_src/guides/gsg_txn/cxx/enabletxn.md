...
source: docs/gsg_txn/CXX/enabletxn.html
---
...
<td><p>You can cause database files to be created in a directory other than the environment home by using the <code class="methodname">DbEnv::add_data_dir()</code> method. The directory identified here must exist. If a relative path is provided, then the directory location is resolved relative to the environment's home directory.</p>
