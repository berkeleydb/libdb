...
#
#	@(#)test008.tcl	10.1 (Sleepycat) 4/12/97
#
# DB Test 8 {access method}
# Take the .o and dbtest files in the current directory and enter their name as
# key with their contents as data.
# After all are entered begin looping through the entries; deleting
# some pairs and then readding them.
...
proc test008 { method {nentries 10000} {reopen 8} {debug 0} args} {
	...
	foreach f $file_list {
		...
		error_check_good Test00$reopen:cmp(./$f,$t4) \
...
proc test008.check { binfile tmpfile } {
	...
	error_check_good cmp(./$binfile,$tmpfile) \
