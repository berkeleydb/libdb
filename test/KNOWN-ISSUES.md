...
| **P2/P3 scope** | **The O_DIRECT defects are FILESYSTEM-DEPENDENT**, discovered by the new G15 gate's own teeth step. They reproduce on **XFS over NVMe**, which enforces `O_DIRECT` buffer alignment strictly, and do **not** reproduce on the GitHub-hosted runner, where `direct_db` reports PASS with `O_DIRECT` confirmed set on the descriptor. Consequence: the strict-mode teeth cannot be a hard gate in either direction on a runner whose filesystem we do not control -- asserting "must fail" breaks on a tolerant fs, asserting "must pass" hides the bug. It is informational on GitHub runners and a real gate in the nightly big-box job, which runs on XFS. This also means the bug is worse than a config nobody uses: it is latent on developer machines and appears on the storage real deployments use. | open |
| **U7** | `--disable-mutexsupport` does not build: `db_atomic_t` is referenced unconditionally in `src/dbinc_auto/os_ext.h`. Found by the new `test/config/option_sweep.sh`; the option had never been built in CI. | open |
