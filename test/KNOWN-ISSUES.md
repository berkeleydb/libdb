...
| id | issue | status |
|----|-------|--------|
| **P5** | **`__log_put`'s single log-region latch is the scaling ceiling, and the critical section is separable.** 56% of all time at t=64, 87% of that on one latch. Anatomy established: `__log_put` holds `LOG_SYSTEM_LOCK` across `__log_put_next` → `__log_putr` → `__log_fill`, and `__log_fill` does the `memcpy` into the shared log buffer while advancing `lp->b_off` (`log_put.c`) — so the **buffer copy is inside the latch**, not just the LSN assignment. That is the whole opportunity: the latch is doing reservation (bump `lp->lsn`/`b_off`) *and* data movement, where only the reservation needs mutual exclusion. **Precedent already exists in this very function**: `__log_putr` accepts a checksum computed by the caller *outside* the latch as an explicit optimization, so splitting work out of this critical section is established practice here. The directly applicable prior art is PostgreSQL's WAL insertion locks (`NUM_XLOGINSERT_LOCKS`, reserve-then-copy-in-parallel) and InnoDB 8.0's redo rewrite (`link_buf`, `recent_written`/`recent_closed`, dedicated writer/flusher threads), both of which removed the global log mutex from the append fast path. Device headroom is enormous: measured 494k IOPS / 1930 MiB/s 4k random write on the test box versus libdb peaking near 203k ops/s at t=2 and falling to ~104k at t=96. Design work tracked in RFC 0008. | open, anatomy done |
