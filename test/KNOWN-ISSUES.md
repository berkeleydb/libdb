...
| id | issue | status |
|----|-------|--------|
| **P4** | **`__db_walk_cursors` wraps the already-sharded cursor queues in the single environment-wide `env->mtx_dblist`, undoing the sharding.** `src/db/db.c:1381` takes `mtx_dblist`, then iterates every handle sharing `adj_fileid` and, within each, all **8** `cq_parts[]` cursor partitions — taking each partition's own `CQ_LOCK` *inside* the global one. So the per-partition locks added by the cursor-sharding work (`DB_CURSOR_NPART = 8`, `db.in:1436`) buy nothing on this path: one global mutex serializes every B-tree insert, because `__bam_iitem` → `__bam_ca_di` → `__db_walk_cursors` runs on each one. Profiled at t=32 on a production (non-DIAGNOSTIC) build: `__bam_ca_di` is **25.58%** of all time, the largest single term, within 62.32% `__db_tas_mutex_lock_int` + 10.27% `__db_tas_mutex_unlock`. Measured shape on 96 vCPU with P1 fixed: peak **203,653 ops/s at t=2**, monotonic decline to **103,669 at t=96** — **49% lost**. It is **not** a "cliff after 8 threads": there is no knee, which is the signature of one global lock rather than a partitioned structure saturating. Note the walk runs even when the handle has no other cursor that could be affected, and there is **no per-handle cursor count** to test cheaply first. Candidates: hold `mtx_dblist` only long enough to snapshot the handle list (it guards the *handle* list, not the cursor queues, which have their own locks); add a cursor count to skip the walk entirely; or key the walk off the affected page. Each needs a cursor-stability argument. | open, root-caused |
