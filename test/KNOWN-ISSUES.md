...
| id | issue | status |
|----|-------|--------|
| **P5** | **`__log_put`'s single log-region latch is the scaling ceiling, but the fix is not the obvious one.** 56% of all time at t=64, 87% of that on one latch; throughput peaks at t=2 (~211k rows/s) and declines to ~90k at t=96 while *device utilisation declines with it* (5.4% -> 2.2% of a measured 494k IOPS / 1930 MiB/s), so the log is not waiting for storage. Anatomy: `LOG_SYSTEM_LOCK` (`log_put.c:176-358`) covers `__log_put_next` -> `__log_putr` -> `__log_fill`, so the latch does both reservation (`lp->lsn` at `:898`, `lp->b_off` at `:1385`) and data movement (the `memcpy` at `:1382`) -- and there is in-function precedent for splitting work out, since `__log_putr` takes a caller-computed checksum (`:844-852`). **But reserve-then-copy addresses a cost this workload does not pay:** `__memmove` is 1.46% of profile against 90.45% for mutex acquire+release, and raising `lg_bsize` 32KB->8MB removed 170x of the `pwrite`s issued *inside* the latch for **no throughput change** (NULL result). A standalone model of the critical section bounds reserve-then-copy at **10-20%** and shows mean hold time inflating **21ns -> ~300ns (14x)** with thread count for *constant work* -- coherence traffic, not compute. What does move: a single-key insert costs **3.10 log records** (2x `__db_addrem` for key and data via `__db_pitem` at `bt_put.c:366`/`:478-483`, plus `__txn_regop`), and batching 4 rows/txn gives **+189% at t=32** (outside a measured +/-22.2% noise floor) while sustaining **2.19x more appends/s on the same latch** -- so the cost is *entering* the serialized stage, not the work inside it. The real invariant is **~90k transactions/sec**, flat as per-txn work varies. A `DB_TXN_NOT_DURABLE` control (full txn + locking, log appends suppressed, verified 0.00 records/row) sizes the prize: the log is **36-62% of removable per-transaction cost at t>=8**, so even a *perfect* append fix buys at most **+57% at t=96**. That control also exposes a **second, non-log scaling defect**: with the log out of the append path throughput still falls from 235k at t=8 to 131k at t=96. Ranked designs, correctness arguments, the multi-process crash hazard that makes reserve-then-copy unsafe here (a reserve-then-die hole is *virtual EOF* per `log_get.c:1238-1240`, silently truncating recovery), and the format-break assessment are in **RFC 0008**, which recommends against its own headline proposal. | open, analysed |
